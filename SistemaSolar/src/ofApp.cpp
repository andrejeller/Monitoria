...
void ofApp::update() {
	...
	//Calcula e movimenta a nave, caso a distancia entre sua posição 
	//e a do clique do mouse seja maior que 2
	if(ofVec2f(ship.pos - mouse).length() >= 2)
		ship.pos = (ship.pos - ((ship.pos - mouse).normalize() * ship.vel));
...
void ofApp::draw() {
	...
	float rotation = ofRadToDeg(atan2f(mouse.y - ship.pos.y, mouse.x - ship.pos.x)) + 90;
